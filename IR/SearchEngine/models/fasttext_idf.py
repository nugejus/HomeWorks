...
class FasttextIDF(Utils):
    def __init__(self):
        super().__init__()
        pass

    def bigram_combination(self, iterable):
        for i in range(len(iterable) - 1):
            yield iterable[i:i+2]

    def bigram_mean_vector(self, word, bigrams):
        word = list(re.sub(r'[^\w]', '', word))
        vec, i = np.zeros(300), 0
        for bi in self.bigram_combination(word):
            bi = "".join(bi)
    ...
    def text_to_vec(self, text, fasttext_vectors, bigrams, term_idx):
        ...
        for word in text:
            if word in fasttext_vectors.keys():
                ...
                vec[term_idx[word]] = self.bigram_mean_vector(word, bigrams)
    ...
    def __call__(self, normalized_requests, normalized_documents, fasttext_vectors, bigrams, term_idx):
        ...
        for request in normalized_requests:
            request_vectors.append(self.text_to_vec(request, fasttext_vectors, bigrams, term_idx))

        document_vectors = []
        for document in normalized_documents:
            document_vectors.append(self.text_to_vec(document,fasttext_vectors,bigrams, term_idx))
